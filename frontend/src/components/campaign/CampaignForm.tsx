...
interface CampaignFormProps {
  ...
  csvPreview: { headers: string[]; rows: Record<string, string>[] } | null;
  setCsvPreview: (v: { headers: string[]; rows: Record<string, string>[] } | null) => void;
...
export function CampaignForm({
  ...
  const [suratMode, setSuratMode] = useState<"upload" | "drive">("upload");
  const [suratDriveLink, setSuratDriveLink] = useState<string>("");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (campaignType === "surat" && suratMode === "upload" && suratFiles.length === 0) {
      toast.error("Harap upload minimal 1 dokumen Surat (PDF) terlebih dahulu.");
      return;
    }

    if (campaignType === "surat" && suratMode === "drive" && !suratDriveLink.trim()) {
      toast.error("Harap masukkan link folder Google Drive surat terlebih dahulu.");
      return;
    }

    const jsonMapping =
      campaignType === "sertifikat"
        ? driveLinksList.reduce(
            (acc, curr) => {
    ...
    if (
      campaignType === "sertifikat" &&
      Object.keys(jsonMapping).length === 0
    ) {
      toast.error(
        "Minimal isi 1 role dan link folder Drive-nya untuk opsi Sertifikat.",
      );
      return;
    }
    ...
    const linkLinkedin =
      ...
      "https://www.linkedin.com/company/gdsc-unsri/";
    const emailAssetBaseUrl =
      import.meta.env.VITE_EMAIL_ASSET_BASE_URL ||
      "https://gdgoc-boilerplate-blast-email.vercel.app/assets";

    const enhancedBody = `
<div style="font-family: Arial, sans-serif; background-color: #f4f4f5; padding: 40px 20px;">
  <div style="max-width: 620px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e4e4e7; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    ...
    <div style="padding: 40px; color: #27272a; font-size: 15px; line-height: 1.6;">
      <div style="margin: 0;">${body.replace(/<p>/g, '<p style="margin: 0.3em 0;">').replace(/<p /g, '<p style="margin: 0.3em 0;" ')}</div>
    </div>
    ...
    if (campaignType === "surat") {
      if (suratMode === "upload" && suratFiles.length > 0) {
        suratFiles.forEach((f) => formData.append("surat_files", f));
      } else if (suratMode === "drive" && suratDriveLink.trim()) {
        // Kirim sebagai folder Drive — worker akan find_file_in_drive() by nama
        formData.set("drive_links_json", JSON.stringify({ default: suratDriveLink.trim() }));
      }
    }
...
<div style="font-family: Arial, sans-serif; background-color: #f4f4f5; padding: 40px 20px;">
  <div style="max-width: 620px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e4e4e7; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    ...
    <div style="background-color: #fafafa; padding: 40px; text-align: center; border-top: 1px solid #e4e4e7;">
      ...
      toast.error(error.message || "Gagal mengirim test email");
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const steps = [
    ...
    {
      id: 4,
      title: campaignType === "sertifikat" ? "Sertifikat" : "Upload Surat",
    },
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="lt-card p-4 sm:p-6 mb-2">
        <div className="flex items-center justify-between relative">
          ...
          <div
            className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-[color:var(--lt-primary)] z-0 rounded-full transition-all duration-500"
            style={{
          ...
          {steps.map((step) => (
            <div
              key={step.id}
              className="flex flex-col items-center gap-2 bg-[color:var(--lt-card)] px-2 z-10"
            >
              <button
                type="button"
              ...
              <span
                className={`text-xs font-semibold hidden sm:block ${activeStep >= step.id ? "text-[color:var(--lt-text-primary)]" : "text-[color:var(--lt-text-secondary)]"}`}
              >
                {step.title}
              </span>
            </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {activeStep === 1 && (
          <div className="lt-card p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h2 className="text-xl font-bold text-[color:var(--lt-text-primary)] mb-6">
              Jenis Campaign & Tema
            </h2>

            <div className="mb-8">
              <label className="block text-sm font-bold text-[color:var(--lt-text-primary)] mb-3">
                Pilih Jenis Lampiran Campaign
              </label>
              <div className="flex flex-col sm:flex-row gap-4">
                <button
                  type="button"
                  onClick={() => {
                    setCampaignType("sertifikat");
                    setSubject("[Certificate of Appreciation] Secure Computer User");
                    setBody(`<p>Hi {{nama}} ✨</p>
...
<p>Best regards,<br><strong>The GDGoC UNSRI 2026 Organizing Team</strong></p>`);
                ...
                >
                  <div className="font-bold text-[color:var(--lt-text-primary)] mb-1">
                    🎓 E-Certificate
                  </div>
                  <span className="text-sm">
                    Kirim sertifikat berbeda untuk setiap nama/role via Google
                    Drive.
                  </span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setCampaignType("surat");
                    setSubject("Official Warning Letter 2 - GDGoC UNSRI Q2");
                    setBody(`<p>Greeting {{nama}} ✨</p>
<br>
<p>This email is to formally inform you that we have issued Warning Letter 2 regarding your participation and contribution as a member of GDGoC UNSRI during Quarter 2 (Q2).</p>
<br>
<p>Attached is the Official Warning Letter 2 , which serves as a formal notice regarding your recent level of engagement within the community. We kindly ask you to review the attached letter carefully.</p>
<br>
<p>We sincerely hope that this serves as an opportunity to strengthen your commitment and involvement in future GDGoC activities. If you have any questions or require further clarification, please do not hesitate to reach out to us.</p>
<br>
<p>Thank you for your attention and cooperation, Folks! 🙌✨</p>
<br>
<p>With lots of gratitude,</p>
<br>
<p>Best regards,</p>
<p><strong>The GDGoC UNSRI 2026 Organizing Team</strong></p>`);
                  }}
                ...
                >
                  ...
                  <span className="text-sm">
                    Kirim lampiran 1 file PDF yang sama ke semua penerima.
                  </span>
                </button>
              </div>
            </div>
            ...
            <div>
                ...
                {["blue", "green", "pink", "yellow"].map((color) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow:
                        themeColor === color
                          ? `0 0 0 4px var(--lt-bg), 0 0 0 6px ${color === "blue" ? "#3b82f6" : color === "green" ? "#22c55e" : color === "pink" ? "#ec4899" : "#eab308"}`
                          : "none",
                      transform:
                        themeColor === color ? "scale(1.1)" : "scale(1)",
                    }}
                    title={`Tema ${color}`}
                  />
                ))}
              ...
              <div className="mt-8 p-4 rounded-xl border border-[color:var(--lt-border)] bg-[color:var(--lt-bg)]">
                <p className="text-sm font-semibold text-[color:var(--lt-text-secondary)] mb-3">
                  Preview Aset Tema
                </p>
                <div className="flex flex-col sm:flex-row items-center gap-6">
                  ...
                  <div className="flex sm:flex-col justify-center items-center gap-4">
                    <p className="hidden sm:block text-xs font-semibold text-[color:var(--lt-text-secondary)] mb-1 text-center">
                      Ikon GDG & Footer
                    </p>
        ...
        {activeStep === 3 && (
          <div className="lt-card p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h2 className="text-xl font-bold text-[color:var(--lt-text-primary)] mb-6">
              Testing Email (Opsional)
            </h2>
            <div className="flex flex-col gap-4 max-w-xl">
              <p className="text-sm text-[color:var(--lt-text-secondary)]">
                Kirim email ujicoba ke email kamu sendiri sebelum blast ke
                banyak orang. Pastikan desain terlihat rapi.
              </p>
          ...
          <div className="lt-card p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {campaignType === "sertifikat" ? (
              <>
                ...
                <DriveLinksList
                  links={driveLinksList}
                  onChange={setDriveLinksList}
                />
              </>
            ) : (
              <>
                ...
                <div className="flex gap-3 mb-6">
                  <button
                    type="button"
                    onClick={() => setSuratMode("upload")}
                    className={`flex-1 py-3 px-4 rounded-xl border-2 text-sm font-semibold transition-all duration-200 ${
                      suratMode === "upload"
                        ? "border-[color:var(--lt-primary)] bg-[color:var(--lt-primary)]/10 text-[color:var(--lt-primary)]"
                        : "border-[color:var(--lt-border)] text-[color:var(--lt-text-secondary)] hover:border-[color:var(--lt-primary)]/50"
                    }`}
                  >
                    📁 Upload File PDF
                  </button>
                ...
                {suratMode === "upload" ? (
                  <div className="flex flex-col gap-4">
                    <div className="flex gap-3 items-start p-4 rounded-xl bg-amber-50 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-700">
                      <span className="text-lg mt-0.5">⚠️</span>
                      <div className="text-sm text-amber-800 dark:text-amber-300">
                        <span className="font-bold">Perhatian:</span> Nama file PDF harus <span className="font-semibold">sama persis</span> dengan kolom <code className="bg-amber-100 dark:bg-amber-800 px-1 rounded font-mono">nama</code> di CSV. Contoh: <code className="bg-amber-100 dark:bg-amber-800 px-1 rounded font-mono">Budi Santoso.pdf</code>.
                      </div>
                    </div>
                    <label
                      className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 ${
                        suratFiles.length > 0
                          ? "border-[color:var(--lt-primary)] bg-[color:var(--lt-primary)]/10"
                          : "border-[color:var(--lt-border)] hover:border-[color:var(--lt-primary)]/50 bg-[color:var(--lt-bg)]"
                      }`}
                    >
                      <input
                        type="file"
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="font-bold text-[color:var(--lt-text-primary)]">
                          Pilih atau Drag beberapa file PDF
                        </span>
                        <span className="text-xs text-[color:var(--lt-text-secondary)] mt-1">
                          Bisa pilih banyak sekaligus · Maks 10MB per file
                        </span>
                      </div>
                    </label>
                    {suratFiles.length > 0 && (
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-semibold text-[color:var(--lt-text-primary)]">
                            {suratFiles.length} file dipilih
                          </p>
                          <button
                            type="button"
                            onClick={() => setSuratFiles([])}
                        ...
                        <div className="flex flex-col gap-1 max-h-48 overflow-y-auto pr-1">
                          {suratFiles.map((f, idx) => (
                            <div
                              key={idx}
                              className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-[color:var(--lt-bg)] border border-[color:var(--lt-border)] text-sm"
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                <span>📄</span>
                                <span className="truncate text-[color:var(--lt-text-primary)] font-medium">{f.name}</span>
                              </div>
                              <button
                                type="button"
                                onClick={() => setSuratFiles((prev) => prev.filter((_, i) => i !== idx))}
                                className="text-red-400 hover:text-red-600 shrink-0 text-xs font-bold"
                              >
                                ✕
                              </button>
                            </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex gap-3 items-start p-4 rounded-xl bg-blue-50 border border-blue-200 dark:bg-blue-900/20 dark:border-blue-700">
                      <span className="text-lg mt-0.5">💡</span>
                      <div className="text-sm text-blue-800 dark:text-blue-300 space-y-1">
                        <p><span className="font-bold">Masukkan link folder Google Drive</span> — sistem otomatis mencocokkan PDF berdasarkan nama file.</p>
                        <p>⚠️ <span className="font-semibold">Nama file PDF di Drive harus sama persis</span> dengan kolom <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded font-mono">nama</code> di CSV. Contoh: <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded font-mono">Budi Santoso.pdf</code>.</p>
                        <p>Pastikan folder dapat diakses siapapun dengan link.</p>
                      </div>
                    </div>
                    <div>
                      ...
                      <input
                        type="url"
                        value={suratDriveLink}
                        onChange={(e) => setSuratDriveLink(e.target.value)}
                        placeholder="https://drive.google.com/drive/folders/..."
                        className="lt-input"
                      />
                    </div>
                    {suratDriveLink && (
                      <div className="flex items-center gap-2 text-sm text-[color:var(--lt-text-secondary)]">
                        <CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" />
                        <span className="truncate">{suratDriveLink}</span>
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="lt-card p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="mt-8 pt-6 border-t border-[color:var(--lt-border)] flex flex-col sm:flex-row justify-between items-center gap-4">
              <button
                type="button"
                onClick={() => setActiveStep(4)}
                className="lt-btn-secondary w-full sm:w-auto"
              >
                Kembali
              </button>
