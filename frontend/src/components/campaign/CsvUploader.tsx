...
interface CsvUploaderProps {
  ...
  setCsvPreview: (preview: { headers: string[]; rows: Record<string, string>[] } | null) => void;
  campaignType?: string;
}
...
export function CsvUploader({
  ...
  const handleClearAll = () => {
    if (
      window.confirm(
        "Apakah Anda yakin ingin menghapus seluruh data di tabel ini?",
      )
    ) {
      setCsvPreview(null);
      setFile(null);
  ...
  const handleClearSuccessful = () => {
    ...
    setCsvPreview(
      newRows.length > 0
        ? { headers: csvPreview.headers, rows: newRows }
        : null,
    );
  };
  ...
  const hasResults = csvPreview?.rows.some(
    (r) => r["_status"] || r["_error_reason"],
  );
  const successCount =
    csvPreview?.rows.filter((r) => r["_status"] === "success").length || 0;
  const failCount =
    csvPreview?.rows.filter((r) => !!r["_error_reason"]).length || 0;

  const displayRows = csvPreview
    ? csvPreview.rows.filter((row) => {
      ...
      Papa.parse(text, {
        ...
        complete: (results) => {
          ...
          if (!csvPreview) {
            ...
            const newRows = [...csvPreview.rows];
            const expectedHeaders = csvPreview.headers.filter(
              (h) => h !== "_status" && h !== "_error_reason",
            );
  ...
  React.useEffect(() => {
    if (!csvPreview) {
      ...
      if (campaignType === "surat" && csvPreview.headers.includes("role")) {
        ...
        !csvPreview.headers.includes("role") &&
        csvPreview.headers.includes("nama") &&
        ...
        const newHeaders = [
          ...
          "email",
          "role",
          ...csvPreview.headers.filter((h) => h !== "nama" && h !== "email"),
        ];
  ...
  const emailCol = csvPreview?.headers.find(
    (h) => h.toLowerCase() === "email" || h.toLowerCase() === "alamat email",
  );
  const emptyEmailsCount =
    csvPreview && emailCol
      ? csvPreview.rows.filter((row) => !String(row[emailCol] || "").trim())
          .length
      : 0;
          ...
          {emptyEmailsCount > 0 && (
            <div className="bg-yellow-500/10 border border-yellow-500/30 text-yellow-700 dark:text-yellow-400 px-4 py-3 rounded-xl text-sm flex items-start gap-3">
              ...
              <div>
                ...
                Terdapat {emptyEmailsCount} baris dengan kolom email yang
                kosong. Baris-baris ini akan dilewati oleh sistem pada saat
                pengiriman.
              </div>
            </div>
          )}
          <div className="border border-border rounded-xl overflow-hidden bg-card shadow-sm transition-colors">
            <div className="px-4 py-3 border-b border-border flex justify-between items-center bg-muted/50 flex-wrap gap-3 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full font-semibold transition-colors">
                  Total: {displayRows.length} baris
                </div>
              </div>
                ...
                {selectedRows.size > 0 && (
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          `Hapus ${selectedRows.size} baris yang dipilih?`,
                        )
                      ) {
                        const newRows = csvPreview.rows.filter(
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      toast.error(
                        "Browser memblokir akses clipboard. Silakan klik di sembarang area tabel lalu tekan Ctrl+V / Cmd+V.",
                      );
                    }
                  }}
            ...
            {hasResults && (
              <div className="flex border-b border-border bg-muted/50 px-4 transition-colors">
                ...
                >
                  Berhasil{" "}
                  <span
                    className={`px-1.5 py-0.5 rounded-full text-[11px] ${activeTab === "success" ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}
                  >
                    {successCount}
                  </span>
                </button>
                ...
                >
                  Gagal{" "}
                  <span
                    className={`px-1.5 py-0.5 rounded-full text-[11px] ${activeTab === "failed" ? "bg-destructive text-destructive-foreground" : "bg-muted text-muted-foreground"}`}
                  >
                    {failCount}
                  </span>
                </button>
                    ...
                    <th className="w-10 p-3 text-muted-foreground font-semibold bg-card text-center transition-colors">
                      <input
                        ...
                        checked={
                          ...
                          displayRows.every((r) =>
                            selectedRows.has(csvPreview!.rows.indexOf(r)),
                          )
                        }
                        onChange={(e) => {
                          if (!csvPreview) return;
                          ...
                          if (e.target.checked) {
                            displayRows.forEach((r) =>
                              next.add(csvPreview.rows.indexOf(r)),
                            );
                          } else {
                            displayRows.forEach((r) =>
                              next.delete(csvPreview.rows.indexOf(r)),
                            );
                          }
                    ...
                    {csvPreview.headers.map(
                      (h, i) =>
                        h !== "_status" && (
                          <th
                            key={i}
                            className={`p-3 font-semibold whitespace-nowrap bg-card transition-colors ${h === "_error_reason" ? "text-destructive" : "text-muted-foreground"}`}
                          >
                          ...
                          (h, j) =>
                            h !== "_status" && (
                              <td
                                key={j}
                                className={
                                  h === "_error_reason" ? "p-3" : "p-0"
                                }
                              >
                                {h === "_error_reason" ? (
                                  <div
                                    className="bg-destructive/20 text-destructive text-[11px] px-2 py-1 rounded font-semibold inline-block max-w-[180px] whitespace-nowrap overflow-hidden text-ellipsis"
                                    title={row[h]}
                                  >
                                    {row[h]}
                                  </div>
