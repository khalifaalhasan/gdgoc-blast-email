...
export default function CampaignList() {
  ...
  if (!isLoaded) return <div className="p-8 text-center text-muted-foreground">Memuat daftar campaign...</div>;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        {campaigns.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 lt-card border-dashed">
            ...
            <h3 className="text-xl font-bold text-[color:var(--lt-text-primary)]">Belum ada campaign</h3>
            <p className="text-[color:var(--lt-text-secondary)] mt-2">Mulai buat campaign pertamamu sekarang.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {campaigns.map((campaign) => (
              <div key={campaign.id} className="group relative lt-card p-5 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                <Link to={`/dashboard/campaign/${campaign.id}`} className="absolute inset-0 z-10 rounded-2xl" aria-label={campaign.name} />
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button 
                    onClick={(e) => {
                      e.preventDefault();
                      if (confirm('Yakin ingin menghapus campaign ini? Semua draft akan hilang.')) {
                ...
                <h3 className="text-lg font-bold mb-1 line-clamp-1 text-[color:var(--lt-text-primary)] tracking-tight" title={campaign.name}>{campaign.name}</h3>
                
                <div className="flex items-center gap-1.5 text-[11px] text-[color:var(--lt-text-secondary)] mt-3 font-medium">
                  <Calendar className="w-3 h-3" />
                  <span>Dibuat pada {new Date(campaign.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                </div>
              </div>
            ))}
          </div>
