...
export default function CampaignCreate() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/dashboard/campaign/${id}/edit`);
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-xl mx-auto pt-2 animate-in fade-in duration-500">
      <div className="mb-6">
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-xs text-[color:var(--lt-text-secondary)] hover:text-[color:var(--lt-text-primary)] mb-3 transition-colors font-medium"
        >
      ...
      <form onSubmit={handleCreate} className="lt-card p-6">
        <div className="mb-6">
          <label className="block text-xs font-bold text-[color:var(--lt-text-primary)] mb-2 uppercase tracking-wider">
            Nama Event / Campaign
          </label>
